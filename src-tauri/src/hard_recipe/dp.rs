...
impl Solver {
    ...
    const TOUCH_SKILLS: [(Actions, u16); 11] = [
    ...
    pub fn read(&mut self, craft_points: i32, durability: u16, buffs: Buffs) -> Option<Actions> {
        // println!("{craft_points} {durability}");
        let result = self.results[craft_points as usize
            * (self.init_status.recipe.durability as usize + 1)
            + durability as usize][buffs.observed as usize][buffs.touch_combo_stage as usize]
            [buffs.great_strides as usize][buffs.manipulation as usize][buffs.innovation as usize]
            [buffs.inner_quiet as usize];
        match result {
            _ if craft_points == 0 || durability == 0 => None,
            ...
            None => {
                ...
                for (action, consumed_du) in Self::TOUCH_SKILLS {
                    if init_status.is_action_allowed(action).is_ok()
                ...
                // println!("best: {:?}", best_action);
                self.results[craft_points as usize
                    * (self.init_status.recipe.durability as usize + 1)
                    + durability as usize][buffs.observed as usize]
                    [buffs.touch_combo_stage as usize][buffs.great_strides as usize]
                    [buffs.manipulation as usize][buffs.innovation as usize]

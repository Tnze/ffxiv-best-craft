...
export async function raphael_solve(status: Status, useManipulation: boolean, backloadProgress: boolean): Promise<Actions[]> {
    clarityReport('runRaphaelSolver')
    const args = { status, useManipulation, backloadProgress };
    if (import.meta.env.VITE_BESTCRAFT_TARGET == "tauri") {
        let { invoke } = await pkgTauri

...
export class BetaXivApiRecipeSource {
    ...
    async recipeTable(page: number, searchName?: string, rlv?: number, craftTypeId?: number): Promise<RecipesSourceResult> {
        ...
        if (filters.length > 0) {
            ...
            const next = async () => {
                ...
                return {
                    results: data.results.map(BetaXivApiRecipeSource.recipeRowsToRecipe),
                    totalPages: 1,
            ...
            // Use list rows api
            const next = async () => {
                const url = new URL('sheet/Recipe', this.base).toString() + '?' + query.toString();
                const resp = await fetch(url, { method: 'GET', mode: 'cors' })
                let data = await resp.json()
                if (!resp.ok) this.checkRespError(data);
                if (data.rows.length > 0) query.set('after', data.rows[data.rows.length - 1].row_id)
                else if (query.has('after')) query.delete('after')
                return {
                    results: data.rows.map(BetaXivApiRecipeSource.recipeRowsToRecipe),
                    totalPages: 1,
    ...
    private static recipeRowsToRecipe(v: any): RecipeInfo {
        return {
            ...
            difficulty_factor: assert(v?.fields?.DifficultyFactor, "difficulty_factor"),
            quality_factor: assert(v?.fields?.QualityFactor, "quality_factor"),
            durability_factor: assert(v?.fields?.DurabilityFactor, "durability_factor"),
            material_quality_factor: assert(v?.fields?.MaterialQualityFactor, "material_quality_factor"),

            required_craftsmanship: assert(v?.fields?.RequiredCraftsmanship, "required_craftsmanship"),
            required_control: assert(v?.fields?.RequiredControl, "required_control"),
    ...
    async recipeLevelTable(rlv: number): Promise<RecipeLevel> {
        ...
        if (this.language != undefined) query.set('language', this.language)
        const url = new URL(`sheet/RecipeLevelTable/${rlv}`, this.base).toString() + '?' + query.toString();
        const resp = await fetch(url, {
            method: 'GET',
            mode: 'cors'
        })
        ...
        return {
            ...
            progress_modifier: assert(data.ProgressModifier, "progress_modifier"),
    ...
    async itemInfo(id: number): Promise<Item> {
        ...
        if (this.language != undefined) query.set('language', this.language)
        const url = new URL(`sheet/Item/${id}`, this.base).toString() + '?' + query.toString();
        const resp = await fetch(url, {
            method: 'GET',
            mode: 'cors'
        })
    ...
    async craftTypeList(): Promise<CraftType[]> {
        ...
        if (this.language != undefined) query.set('language', this.language)
        const url = new URL(`sheet/CraftType`, this.base).toString() + '?' + query.toString();
        const resp = await fetch(url, {
            method: 'GET',
            mode: 'cors'
        })
        ...
        return data.rows.map((v: any) => <CraftType>{
    ...
    private async getEnhancers(_page: number, categoryID: number): Promise<DataSourceResult<Enhancer>> {
        ...
        return {
            ...
            results: data.flatMap(item => {
                const itemFood = itemFoodsMap.get(item.fields.ItemAction.fields.Data[1]) // [1] ItemFood (row_id), [2] Duration (sec)
                return itemFood?.map(v => ({ ...v, name: item.fields.Name + v.name })) ?? []
            }),
        }
    }
    ...
    private static itemFoodToEnhancer(name: string, itemFood: any): [Enhancer, Enhancer] {
    ...
    private async queryAll<T>(sheets: string, query: string, fields?: string) {
        ...
        do {
            const url = new URL(`search`, this.base).toString() + '?' + params.toString();
